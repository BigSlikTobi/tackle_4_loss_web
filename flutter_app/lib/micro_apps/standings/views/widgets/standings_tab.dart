...
class _StandingsTabState extends State<StandingsTab> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _scrollSubscription = controller.scrollRequests.listen(_handleScrollRequest);
    });
  }
  ...
  void _handleScrollRequest(String key) {
      ...
      if (controller.selectedConference != null) {
          final specificKey = "${controller.selectedConference} $normalizedInfo".toUpperCase();
    ...
    return Consumer<StandingsController>(
      builder: (context, controller, child) {
        if (controller.isLoadingStandings) {
          return const Center(child: CircularProgressIndicator(color: AppColors.brandBase));
        }
            ...
            slivers: [
              ...
              SliverPersistentHeader(
                ...
                delegate: _StandingsHeaderDelegate(
                  ...
                  height: controller.viewMode == StandingsViewMode.division ? 160 : 110, 
                ),
              ),
  ...
  Widget _buildLeagueView(List<ConferenceStandings> standings) {
    ...
    // Sort by record
    allTeams.sort((a, b) {
        if (a.winPercentage != b.winPercentage) return b.winPercentage.compareTo(a.winPercentage);
  ...
  Widget _buildConferenceView(List<ConferenceStandings> standings, StandingsController controller) {
      return SliverToBoxAdapter(
          child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
  ...
  Widget _buildDivisionView(List<ConferenceStandings> standings, StandingsController controller) {
       final items = <Widget>[];
       ...
       for (var conf in standings) {
           ...
           for (var div in conf.divisions) {
               ...
               var sectionName = "${conf.conference.toUpperCase()} ${div.division.toUpperCase()}";
               if (div.division.toUpperCase().contains(conf.conference.toUpperCase())) {
  ...
  Widget _buildErrorState(BuildContext context, StandingsController controller) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          const Icon(Icons.error_outline, size: 48, color: AppColors.breakingNewsRed),
          const SizedBox(height: AppSpacing.space2),
          const Text('Failed to load standings', style: AppTextStyles.h3),
          const SizedBox(height: AppSpacing.space1),
          TextButton(onPressed: controller.fetchStandings, child: const Text("Retry")),
        ],
      ),
    );
  }
