...
void main() {
  ...
  group('NewContentService', () {
    test('hasNewDeepDive returns false when no latest article is set', () async {
      final service = NewContentService.testing();
      expect(service.hasNewDeepDive, isFalse);
    });

    test('hasNewDeepDive returns true when new article ID differs from last seen', () async {
      SharedPreferences.setMockInitialValues({
    ...
    test('markDeepDiveSeen persists to SharedPreferences', () async {
      ...
      expect(prefs.getString('last_seen_deep_dive_id'), equals('persist-test-id'));
    });
